...
/*   Updated: 2023/03/31 15:24:40 by jsauvain         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::incrementGrade(void)
{
	if (_grade - 1 < 1)
	{
		std::string	tmp = "Cannot upgrade " + _name + "'s grade";
		throw GradeTooHighException(tmp);
	}
	_grade--;
}
...
void	Bureaucrat::decrementGrade(void)
{
	if (_grade + 1 > 150)
	{
		std::string	tmp = "Cannot downgrade " + _name + "'s grade";
		throw GradeTooLowException(tmp);
	}
	_grade++;
}

void	Bureaucrat::signForm(Form & source) const
{
	try
	{
		if (source.getSigned())
			throw (_name + " couldn't sign because the form was already signed");
